...
export default function App() {
  ...
  // 비로그인 상태에서 "찻장에 담기"를 누르면 로그인을 먼저 유도.
  // 담으려던 기록은 여기 잠시 보관했다가, 로그인 성공 시 이어서 저장.
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      // 로그인에 성공하면 로그인 팝업은 항상 닫는다 (직접 로그인이든, 담기 게이트를 거쳤든)
      if (u) {
        setAuthOpen(false)
        // 담기 게이트를 거쳐 로그인한 경우, 보류해둔 기록을 이어서 저장
        if (pendingRef.current) {
          const rec = pendingRef.current
          pendingRef.current = null
          setPendingRec(null)
          await persistRecord(u.uid, rec)
        }
      }
    })
    return unsub
  ...
  // 실제 저장 + 후처리(마일스톤/탭 이동). uid=null이면 이 기기에만 저장.
  async function persistRecord(uid, rec) {
    await saveRecord(uid, rec)
    const next = await loadRecords(uid)
    setRecords(next)
    setRecordFor(null)
    const hit = MILESTONES.find((m) => m.count === next.length)
    if (hit) setMilestone(hit)
    else switchTab('archive')
  }

  // 시트의 "찻장에 담기" 진입점.
  // 로그인 상태면 바로 저장. 비로그인이면 로그인 팝업을 먼저 띄우고 기록은 보류.
  async function handleSave(rec) {
    if (user) { await persistRecord(user.uid, rec); return }
    pendingRef.current = rec
    setPendingRec(rec)
    setAuthOpen(true)
  }

  // 로그인 팝업에서 "로그인 없이 이 기기에만 저장"을 고른 경우
  async function saveLocalOnly() {
    const rec = pendingRef.current
    pendingRef.current = null
    setPendingRec(null)
    setAuthOpen(false)
    if (rec) await persistRecord(null, rec)
  }

  function closeAuth() {
    // 로그인 팝업을 그냥 닫으면 보류 기록은 시트로 되돌림(사용자가 다시 선택 가능)
    pendingRef.current = null
    setPendingRec(null)
    setAuthOpen(false)
  }
    ...
    <div className="app">
      <div className="topbar">
        <header className="hdr">
          <button className="brand" onClick={() => switchTab('today')} aria-label="홈으로">
            <BrandMark />
            <span className="logotype">픽티업<span className="with">with 핑티</span></span>
          </button>
          {user ? (
            <button className="btn-login account" onClick={() => setLogoutOpen(true)}>
              <span className="acc-dot" />
              {user.email ? user.email.split('@')[0] : '내 계정'}
            </button>
          ) : (
            <button className="btn-login" onClick={() => setAuthOpen(true)}>로그인</button>
          )}
        </header>
      ...
      <main className="pad" style={{ paddingTop: 18 }}>
        ...
        {tab === 'archive' && (
          <ArchiveView
            ...
            onGoToday={() => switchTab('today')}
          />
        )}
        {tab === 'guide' && <GuideView onRecord={setRecordFor} />}

        <div className="footer-links">
          <button className="link-pill" onClick={() => setOnboarding(true)}>
            <span className="spark">✦</span> 핑티 이야기 다시 보기
          </button>
          <button className="link-pill sub" onClick={() => setAboutOpen(true)}>
            <span className="spark">✦</span> by EMMA
          </button>
        </div>
      </main>
      ...
      {authOpen && (
        <Overlay onClose={closeAuth} align="center">
          <AuthPanel
            title={pendingRec ? '로그인하고 찻장에 담기' : '로그인하고 계속하기'}
            note={pendingRec
              ? '로그인하면 이 기록이 계정에 저장돼, 어느 기기에서든 이어서 볼 수 있어요.'
              : '기록을 계정에 저장하면, 어느 기기에서든 이어서 볼 수 있어요.'}
          />
          {pendingRec && (
            <button className="ghost u" style={{ width: '100%', marginTop: 14 }} onClick={saveLocalOnly}>
              로그인 없이 이 기기에만 저장할게요
            </button>
          )}
        </Overlay>
      ...
      {milestone && (
        <MilestoneModal
          data={milestone}
          onClose={() => { setMilestone(null); switchTab('archive') }}
        />
      )}
...
function Onboarding({ onDone, revisit = false }) {
  ...
  return (
    <div className="app">
      <div className="topbar">
        <header className="hdr" style={{ paddingBottom: 12 }}>
          <div className="brand">
            <BrandMark />
            <span className="logotype">픽티업<span className="with">with 핑티</span></span>
          </div>
      ...
      <div className="pad" style={{ paddingTop: 8 }}>
        ...
        <div style={{ marginTop: 14, display: 'grid', gap: 8 }}>
          <button className="btn" onClick={() => (last ? onDone() : go(1))}>
            {last ? (revisit ? '차 마시러 가기' : '차 마시러 가기 ✦') : '다음'}
          </button>
    ...
    <section>
      ...
      {records.length === 0 ? (
        <div className="card" style={{ textAlign: 'center' }}>
          ...
          <button className="btn" onClick={onGoToday}>차 골라보기</button>
        </div>
      ) : (
        shown.map((r) => {
...
function TasteMap({ records }) {
  ...
  return (
    <div className="card" style={{ marginBottom: 12 }}>
      ...
      <p className="tiny" style={{ marginTop: 10 }}>자주 고른 맛일수록 길어져요. 취향은 이렇게 쌓여가요.</p>
    </div>
  )
}
...
function GuideView({ onRecord }) {
  ...
  // 목록 ↔ 카테고리 ↔ 상세 이동 시 이전 스크롤 위치가 남지 않게
  useEffect(() => { window.scrollTo(0, 0) }, [openCat, detail])
  ...
  return (
    <section>
      ...
      <p className="hint-swipe" style={{ marginBottom: 8 }}>옆으로 넘겨보세요 →</p>
      <div className="hscroll">
        {CATEGORY_ORDER.map((k) => {
          const c = categoryOf(k)
          return (
            <button className="cover h" key={k} onClick={() => setOpenCat(k)}>
      ...
      <div style={{ marginTop: 30 }}>
        ...
        <div className="hscroll">
          {TEAWARE.map((t) => (
            <div className="ware-card" key={t.id}>
              <img src={`${import.meta.env.BASE_URL}illustrations/${t.art}.jpg`} alt="" loading="lazy" />
              <div className="ware-body">
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, flexWrap: 'wrap' }}>
                  <div className="h3">{t.name}</div>
                  <div className="tiny">{t.sub}</div>
                </div>
                <p className="wrap-safe" style={{ marginTop: 4, fontSize: 12, lineHeight: 1.65 }}>{t.body}</p>
              </div>
            </div>
          ))}
        </div>
...
/* ── 기록 시트 ───────────────────────────── */
function RecordSheet({ tea, user, onClose, onSave }) {
  ...
  return (
    <Overlay onClose={onClose} align="bottom">
      ...
      <textarea
        ...
        placeholder="오늘의 차, 한 줄로 남겨보세요."
        style={{ marginBottom: 14 }}
      />

      {!user && (
        <p className="tiny" style={{ marginBottom: 12 }}>
          찻장에 담을 때 로그인하면 계정에 안전하게 저장돼요. 로그인 없이 이 기기에만 담아둘 수도 있어요.
        </p>
      )}
  ...
  return (
    <Overlay onClose={onClose} align="center">
      <div style={{ textAlign: 'center' }}>
        ...
        <div className="wrap-safe" style={{ textAlign: 'left', margin: '12px 0 20px', display: 'grid', gap: 11 }}>
          <p className="body">차를 처음 우려준 사람은 엄마였습니다. 오래전 엄마는 다도를 했고, 저는 그 곁에서 찻물 식는 냄새를 맡으며 자랐어요.</p>
          <p className="body">어른이 되어 혼자 살기 시작하며 다시 차를 우렸습니다. 물 온도도, 잎의 양도, 제가 뭘 좋아하는지도 몰라서 자주 머뭇거렸어요. 그 머뭇거림을 기억하려고 이 앱을 만들었습니다. 처음의 막막함을 지나, 좋아하는 맛을 찾아가는 길이 조금 덜 외롭도록.</p>
          <p className="body">지금은 친구들과 찻자리를 벌이기도 하고, 혼자서도 곧잘 우려 마십니다. 그래도 차가 주는 가장 좋은 일은 변하지 않았어요. 숨을 깊게 쉬게 되고, 몸이 따뜻해지는 것. 찻자리의 편안한 기운은 덤이고요.</p>
          <p className="muted">한 잔 곁에 두고, 천천히 골라보세요.</p>
        </div>
