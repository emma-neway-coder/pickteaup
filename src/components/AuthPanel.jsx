...
export default function AuthPanel({ title = '로그인하고 계속하기', note }) {
  ...
  // 로그인 성공 후 앱이 화면을 전환할 때까지의 짧은 공백에 빈 폼이 번쩍이지 않도록,
  // 성공하면 이 상태를 유지해 "확인 중" 표시를 계속 보여준다. (에러일 때만 폼 복귀)
  const [done, setDone] = useState(false)
  const [err, setErr] = useState('')

  async function google() {
    setErr(''); setBusy(true)
    try { await signInWithPopup(auth, googleProvider); setDone(true) }
    catch (e) { setErr(msg(e.code)); setBusy(false) }
  }
  ...
  // 로그인 성공 후 앱이 팝업을 닫기까지의 짧은 순간 — 빈 폼 대신 확인 중 표시
  if (done) {
    return (
      <div style={{ maxWidth: 340, margin: '0 auto', textAlign: 'center', padding: '20px 0' }}>
        <div className="h2" style={{ marginBottom: 8 }}>로그인됐어요</div>
        <p className="muted">잠시만요, 찻자리를 준비하고 있어요…</p>
      </div>
    )
  }
