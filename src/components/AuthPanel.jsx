...
export default function AuthPanel({ title = '로그인하고 계속하기', note }) {
  ...
  return (
    <div style={{ maxWidth: 360, margin: '0 auto' }}>
      <div className="h2" style={{ textAlign: 'center', marginBottom: note ? 6 : 16 }}>{title}</div>
      {note && <p className="muted" style={{ textAlign: 'center', marginBottom: 18 }}>{note}</p>}

      <button className="btn-plain" onClick={google} disabled={busy}>
        <GoogleG /> 구글로 계속하기
      </button>

      <div className="divider">또는</div>

      <input className="field" style={{ marginBottom: 8 }} type="email" inputMode="email"
        autoComplete="email" placeholder="이메일"
        value={email} onChange={(e) => setEmail(e.target.value)} />
      <input className="field" style={{ marginBottom: 12 }} type="password"
        autoComplete={mode === 'signup' ? 'new-password' : 'current-password'}
        placeholder="비밀번호 (6자 이상)"
        value={pw} onChange={(e) => setPw(e.target.value)} />

      <button className="btn" onClick={byEmail} disabled={busy}>
        {busy ? '잠시만요…' : mode === 'signup' ? '이메일로 가입' : '이메일로 로그인'}
      </button>

      {err && <p className="tiny" style={{ color: 'var(--camel)', textAlign: 'center', marginTop: 10 }}>{err}</p>}
