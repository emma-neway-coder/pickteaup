...
/* 헤더 우측 — 로그인/계정 글래스 필 */
/* ── 계정 필 (우상단) ──
   로그인 전 = 연한 비취 도자기 유약 버튼(행동 유도)
   로그인 후 = 조용한 필 + 비취 점(상태 표시)  */
.btn-login{
  flex:none;max-width:150px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;
  font-size:12.5px;color:#2C4038;padding:8px 17px;border-radius:12px;border:none;
  position:relative;isolation:isolate;
  background:
    radial-gradient(140% 120% at 50% -30%, rgba(255,255,255,.75), rgba(255,255,255,0) 60%),
    linear-gradient(178deg,#D2E1DA 0%, #BBD2C8 55%, #A8C6BA 100%);
  box-shadow:
    0 1px 0 rgba(255,255,255,.7) inset,
    0 -3px 10px rgba(150,185,172,.35) inset,
    0 3px 9px rgba(150,185,172,.28), 0 1px 3px rgba(150,185,172,.2);
  transition:transform .13s var(--ease), box-shadow .13s var(--ease);
}
/* 도자기 유약처럼 위에서 넓게 번지는 광택 */
.btn-login::before{
  content:"";position:absolute;inset:0;z-index:-1;border-radius:inherit;pointer-events:none;
  background:radial-gradient(90% 75% at 50% -5%,
    rgba(255,255,255,.7), rgba(255,255,255,.12) 45%, rgba(255,255,255,0) 72%);
}
.btn-login:active{transform:translateY(1px) scale(.97);
  box-shadow:0 1px 0 rgba(255,255,255,.55) inset, 0 -2px 6px rgba(150,185,172,.3) inset,
    0 2px 5px rgba(150,185,172,.24)}
/* 로그인 후 계정 상태 — 조용한 필 */
.btn-login.account{
...
.btn-login.account::before{display:none}
.btn-login.account .acc-dot{width:7px;height:7px;border-radius:50%;flex:none;background:#7BA99C}
.btn-login.account:active{transform:scale(.97);
  box-shadow:0 1px 0 rgba(255,255,255,.9) inset, 0 1px 2px rgba(74,59,46,.06)}

/* ── 탭 — 옅은 트랙 + 연한 비취 활성 카드 (알약 아님) ── */
.tabs{display:flex;padding:4px;margin:0 18px 13px;border-radius:13px;
  background:rgba(240,233,219,.7);border:1px solid rgba(228,221,206,.6);
  box-shadow:inset 0 1px 3px rgba(74,59,46,.05)}
.tab{
  flex:1;padding:10px 0 11px;font-size:13px;font-family:${serif};border:none;
  border-radius:10px;color:var(--umber-soft);background:none;position:relative;
  transition:transform .16s var(--ease), background .2s var(--ease),
    color .2s ease, box-shadow .2s ease, font-weight .2s ease;
}
.tab:active{transform:scale(.97)}
.tab.on{
  color:#28352C;font-weight:700;
  background:linear-gradient(180deg,#FBFBF6,#EDF2EC);
  box-shadow:0 1px 0 rgba(255,255,255,.9) inset, 0 2px 7px rgba(74,59,46,.09);
}
...
/* ── 세이지 유약 버튼 (찻잎 비대칭) ──
   유약감은 "선"이 아니라 표면 전체에 흐르는 광택으로 만든다.
   ::before = 위에서 아래로 넓게 번지는 글레이즈(유약이 고이는 결)
   ::after  = 상단 가장자리에 얇게 맺히는 스펙큘러(도자기 광택)         */
.btn{
  width:100%;font-size:14px;color:#2E3826;border:none;position:relative;
  isolation:isolate;overflow:hidden;
  padding:14px 22px;border-radius:24px 24px 24px 7px;
  background:
    radial-gradient(120% 80% at 50% -10%, rgba(255,255,255,.5), rgba(255,255,255,0) 55%),
    linear-gradient(176deg,#C2CCB6 0%, #AAB79C 46%, #97A589 78%, #8A997C 100%);
  box-shadow:
    0 1px 0 rgba(255,255,255,.55) inset,
    0 -6px 12px rgba(120,138,108,.4) inset,
    0 8px 18px rgba(126,142,114,.32), 0 2px 5px rgba(126,142,114,.24);
  transition:transform .14s var(--ease), box-shadow .14s var(--ease);
}
/* 유약이 위에서 흘러내린 듯한 넓은 광택 면 */
.btn::before{
  content:"";position:absolute;inset:0;z-index:-1;border-radius:inherit;
  background:linear-gradient(180deg,
    rgba(255,255,255,.55) 0%, rgba(255,255,255,.16) 26%,
    rgba(255,255,255,0) 52%, rgba(120,138,108,.14) 100%);
  pointer-events:none;
}
/* 상단 가장자리에 부드럽게 맺히는 광택(선이 아니라 번짐) */
.btn::after{
  content:"";position:absolute;left:8%;right:8%;top:0;height:52%;z-index:-1;
  border-radius:inherit;
  background:radial-gradient(90% 100% at 50% 0%,
...
.btn:active{
  ...
  box-shadow:
    ...
    0 -3px 7px rgba(120,138,108,.34) inset,
    0 3px 8px rgba(126,142,114,.26);
}
.btn:active::before{background:linear-gradient(180deg,
  rgba(255,255,255,.38) 0%, rgba(255,255,255,.1) 30%,
  rgba(120,138,108,.18) 100%)}
/* 진한 배리에이션 — 보조 */
.btn.deep{
  background:
    radial-gradient(120% 80% at 50% -10%, rgba(255,255,255,.34), rgba(255,255,255,0) 55%),
    linear-gradient(176deg,#93A385,#7E8E72 58%,#6C7B61 100%);
  color:#F4F6F0;
  box-shadow:
    0 1px 0 rgba(255,255,255,.34) inset,
    0 -6px 12px rgba(88,102,78,.46) inset,
    0 8px 18px rgba(90,104,80,.34), 0 2px 5px rgba(90,104,80,.26);
}
.btn.deep::before{background:linear-gradient(180deg,
  rgba(255,255,255,.34) 0%, rgba(255,255,255,.08) 30%,
  rgba(255,255,255,0) 55%, rgba(88,102,78,.2) 100%)}
...
/* by EMMA 초상 — 배경을 시트에 녹여 라인만 남긴다(multiply) */
.about-portrait{
  width:104px;height:104px;display:block;margin:0 auto 12px;border-radius:24px;
  mix-blend-mode:multiply;
}
@supports not (mix-blend-mode:multiply){
  .about-portrait{border-radius:24px;box-shadow:0 2px 8px rgba(74,59,46,.1)}
}
